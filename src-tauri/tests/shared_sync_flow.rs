...
fn bootstrap_writes_one_operation_per_existing_entry_once() {
    ...
    assert_eq!(operation_file_count(&shared_root), 2);
...
fn scripted_one_machine_smoke_uses_env_shared_root() {
    ...
    let db_a =
        InventoryDb::open_at(smoke_root.join("client-a").join("inventory.feox"), None).unwrap();
    let db_b =
        InventoryDb::open_at(smoke_root.join("client-b").join("inventory.feox"), None).unwrap();
