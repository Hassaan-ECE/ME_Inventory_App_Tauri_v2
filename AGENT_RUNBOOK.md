...
- Keep importing files and extracted folders together. If `InventoryShell.tsx` imports `src/components/inventory/shell/*`, that folder must be included when staging or committing.
...
- Pivot: confirm no app/build processes are running, remove only the generated installer for the target version under `src-tauri\target\release\bundle\nsis\`, then rerun the bundle command.
...
& "$env:USERPROFILE\.bun\bin\bun.exe" run test -- src/test/inventory-shell.test.tsx src/test/entry-dialog.test.tsx
...
- The manager updates `CLEANUP_CHECKLIST.md` after each worker result.
